...
function renderMemberRunsTable(memberName) {
  ...
  const raceRows = memberRaces.flatMap(race =>
    race.games.map(game => ({
      ...
      RUNNER7: race.runners[6]?.name || "",
      RUNNER8: race.runners[7]?.name || "",
      RUNNER9: race.runners[8]?.name || "",
      RUNNER10: race.runners[9]?.name || "",
      RUNNER11: race.runners[10]?.name || "",
      RUNNER12: race.runners[11]?.name || "",
      RUNNER13: race.runners[12]?.name || "",
      RUNNER14: race.runners[13]?.name || "",
      RUNNER15: race.runners[14]?.name || "",
      RUNNER16: race.runners[15]?.name || "",
      RUNNER17: race.runners[16]?.name || "",
      RUNNER18: race.runners[17]?.name || "",
      RUNNER19: race.runners[18]?.name || "",
      RUNNER20: race.runners[19]?.name || "",
      RUNNER21: race.runners[20]?.name || "",
      COMMENTARY1: "",
      COMMENTARY2: "",
      COMMENTARY3: "",
      COMMENTARY4: "",
